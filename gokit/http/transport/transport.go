...
func NewTransport() *mux.Router {
	pwd, err := os.Getwd()
	if err != nil {
		panic(err)
	}
	r := mux.NewRouter().StrictSlash(true)
	fs := justFilesFilesystem{fs: http.Dir(pwd + "/static"), readDirBatchSize: 2}
	interfaces.RouterOptions(r,
		routers.NewAnalystRouter,
	)
	r.PathPrefix("/").Handler(http.StripPrefix("/", http.FileServer(fs)))
	r.Use(middleware.ApiLoggingMiddleware)
