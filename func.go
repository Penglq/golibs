...
func GetClusterCache(key string, t time.Duration, res interface{}, f func() (interface{}, error)) (err error) {
	...
	if errs != nil && errs != redis.Nil {
		QLog.GetLogger().AlertWithLevel(QLog.ALERTWARNING, "redis缓存", "redis get 出错", "key", key, "错误信息", errs.Error())
	}

	if rs != "" {
		errs = json.Unmarshal([]byte(rs), res)
		if errs != nil {
			QLog.GetLogger().AlertWithLevel(QLog.ALERTWARNING, "redis缓存", "json 错误", "数据", rs, "错误信息", errs.Error())
		} else {
			return
		}
	}
	...
	if errs != nil {
		QLog.GetLogger().AlertWithLevel(QLog.ALERTWARNING, "redis缓存", "json 错误", "错误信息", errs.Error())
	} else {
		_, errs = cache.GetCache().Set(key, string(b), t).Result()
		if errs != nil {
			QLog.GetLogger().AlertWithLevel(QLog.ALERTWARNING, "redis缓存", "redis set 出错", "key", key, "data", string(b), "错误信息", errs.Error())
		}
		// QLog.GetLogger().Info("json数据>>>>>>", string(b))
	}
